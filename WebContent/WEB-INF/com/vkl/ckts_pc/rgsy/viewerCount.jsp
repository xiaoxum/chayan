...
<meta http-equiv="X-UA-Compatible" content="IE=edge"><!--图表兼容IE必加标签-->
<title>查验员合格率统计</title>
...
function page(num){
	...
	var dd = {organCode : chkpt,
			...
			pageSize : 10};
	$.ajax({
		url : '<%=request.getContextPath()%>/count/viewer/viewerPage ',
		type : 'post',
		data : dd,
...
function chart(dt){
	...
	$.each(dt,function(index,dd){
		viewer[index] = dd.viewer;
		value[index] = dd.rate;
	});
	for(var i=viewer.length; i<10; i++){
		viewer[i]="";
	} 
