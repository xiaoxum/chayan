...
<body>
	...
	<div class="easyui-panel" style="width:100%; height:auto;" title="新增角色">
			<table width="100%" cellspacing="0" cellpadding="0">
            <tr>
             <TD width="200" class="bgc"><span class="star">*</span>角色名称</TD><TD colspan="3"> <input id="roleName" class="easyui-textbox" style="width:80%;"/></TD>
             
           </tr>
           <TR>
            <TD class="bgc"><span class="star">*</span>任务描述</TD><TD colspan="3"><input id="roleIntr" class="easyui-textbox" style="width:80%;"/></TD>
           </TR>
           <TR>
            <TD class="bgc"><span class="star">*</span>权限名称</TD>
            <TD colspan="3">
             <select class="easyui-combotree" data-options="multiple: true,cascadeCheck:false,onCheck:checkNode" url="<%=request.getContextPath()%>/module/role/findMenuList?parentMenu=0" id="menuId" style="width:80%;"></select>
            </TD>
           </TR>
           <TR>
...
function checkNode(node){
	  var  node1=$(tree).tree('getParent',node.target);          //得到父节点
	    $("#cdid").tree('check', node1.target);                                      //选中父节点
}
	...
	$.ajax({
		...
		success:function(result){
			...
			if(result.data=="true"){
				...
				$.ajax({
					url:'<%=request.getContextPath()%>/module/role/submit',
					data:{
						roleName: roleName,
						roleIntr: roleIntr,
					...
					success:function(result){
						...
						if(result.data=="true"){
							$.messager.confirm("提示！", "角色添加成功，是否继续添加？",
					                function(r) {
					                    if (r) {
					                    	window.location = '<%=request.getContextPath()%>/module/role/jsadd';
					                    } else {
