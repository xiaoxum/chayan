...
<body onunload="unload()">
	...
	<div
		...
		<button  type="button" class="easyui-linkbutton" data-options="iconCls:'icon-edit'"  onclick="gaopai();">资料高拍</button>
		<button type="button" class="easyui-linkbutton"
...
function startRequest(){
	$.ajax({
		url : xmlj + "/module/vehicleinfo/getnodyjl",
		cache : false,
		async : false,
		success : function(data) {
			if (data == "no-login") {
				top.location = xmlj + "/login.jsp"
			}
			if (data != null) {
				$.each(data, function(index, da) {
					
					dy4(da.srln, '5', da.rckCount, da.syxzx,da.auditFlag, da.cllxs);
				})
			}
...
function jump(srln,rckCount) {
	...
	var url="<%=request.getContextPath()%>/module/vehicleinfo/loadcheckaudit?srln="+srln+"&rckCount="+rckCount;
...
function dy1(clsbdh,csys,srln,cllxName,hpzlName,hphm,rckCount){
	Report.LoadFromURL(xmlj + "/report/pz.grf");
	Report.LoadDataFromURL(xmlj + "/module/login/getbzxx?clsbdh=" + clsbdh
			+ "&csys=" + csys+"&srln="+srln+"&cllxName="+cllxName+"&hpzlName="+hpzlName+"&hphm="+hphm+"&rckCount="+rckCount);
	Report.Print(false);
}
...
function dy3(srln,statu,rckCount){
	if(statu=="3"||statu=="4"||statu=="7"){
		alert("当前业务未结束，不能打印查验记录");
		return;
	}
	$.ajax({
		type: "POST",
        	...
        	for(var i=0;i<data.list.length;i++){
        		...
        		if(i % 2 == 1){
					var tr="<tr ondblclick='jump("+data.list[i].srln+","+data.list[i].rckCount+")'class='rowbgcolor '>"
					    +"<td class='texcenter'><input  type='radio' name='radio' value="+data.list[i].srln+":"+data.list[i].rckCount+":"+data.list[i].operStatu+":"+data.list[i].auditFlag+" /></td>"
			         ...
			         /* 	+"<td>"+data.list[i].userName+"</td>" */
			         	...
			         	+"<td>"+sji+"</td>"
			         	+"<td>"+Y+M+D+"</td>"
			         	+"<td ><button  type='button' onClick='dy1(\""+data.list[i].clsbdh+"\",\""+data.list[i].csys+"\",\""+data.list[i].srln+"\",\""+data.list[i].cllxName+"\",\""+data.list[i].hpzlName+"\",\""+data.list[i].hphm+"\",\""+data.list[i].rckCount+"\")'>凭证打印</button><button  type='button' onClick='dy4("+ data.list[i].srln
						+ ","
						+ data.list[i].operStatu
						+ ","
						+ data.list[i].rckCount
						+ ",\""
						+ data.list[i].syxz
						+ "\",\""
						+ data.list[i].auditFlag
						+ "\",\""   
						+ data.list[i].cllx
						+ "\")'>报告打印</button><button  type='button' onClick='fj("+data.list[i].srln+","+data.list[i].operStatu+","+data.list[i].rckCount+","+data.list[i].auditFlag+")'>复检</button></td>"
			         	+"</tr>";
				}else{
					var tr="<tr ondblclick='jump("+data.list[i].srln+","+data.list[i].rckCount+")'  >"
						...
						+ "\")'>报告打印</button><button  type='button' onClick='fj("+data.list[i].srln+","+data.list[i].operStatu+","+data.list[i].rckCount+","+data.list[i].auditFlag+")'>复检</button></td>"
			         	+"</tr>";
			         	/* <button  type='button' onClick='dy2("+data.list[i].srln+","+data.list[i].operStatu+","+data.list[i].rckCount+")'>照片打印</button> */
				}
        		// 添加tr
        		$("#myTbody").append(tr);
        	}
...
function dy4(srln, statu, rckCount, syxz,auditFlag,cllxs) {
	if (statu == "3") {
		alert("当前业务未查验完，不能打印查验记录");
		return;
	}
	if(auditFlag != '0'){
		alert("当前业务未审核或审核不通过，不能执行打印操作");
		return;
	}
	if ("K38" == cllxs || "K28" == cllxs || "K18" == cllxs || 'O' == syxz
			|| 'P' == syxz || 'S' == syxz || 'Q' == syxz) {
		Report.LoadFromURL(xmlj + "/report/xccy.grf");
	} else {
		Report.LoadFromURL(xmlj + "/report/jrb.grf");
	}
	 try {
		 Report.LoadDataFromURL(xmlj + "/module/login/getdysj?srln=" + srln
					+ "&rckCount=" + rckCount+ "&time=" +(new Date()));
			Report.Print(false);
     }
     catch (e) {
         return;
...
function toStr(str){
 ...
 function deleteCkInfo(){
		...
		if(operStatu[2]=='5'||operStatu[2]=='4'){
			$.messager.alert("提示！", "当前业务已受理成功，不能执行删除操作");
			return;
		}
		/* if(operStatu[3]=='0'){
			$.messager.alert("提示！", "当前业务已受理成功，不能执行删除操作");
		...
		parent.$.messager.confirm('询问', '您是否删除此查验信息？',
				function(b) {
					if (b) {
 ...
 function updateCkInfo(){
		...
		var operStatu=ckid.split(":");
		if(operStatu[2]!='3'){
			$.messager.alert("提示！", "当前业务已已完成或正在审核，不能修改");
			return;
		}
     ...
     function gaopai() {
	     var  ckid=$("[name=radio]:checked").val();
	     var ckids=ckid.split(':');
	     var srln=ckids[0];
	     var rckCount=ckids[1];
	     if(ckids[2] != '6'){
				$.messager.alert("提示！", "当前操作不当，无需高拍");
				return;
		 }
		parent.gaopai(srln, rckCount);
	}
